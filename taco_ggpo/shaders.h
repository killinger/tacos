...
struct vertex_shader
{
	...
	void Initialize(ID3D11Device* Device)
	{
		...
		Result = D3DCompileFromFile(L"Shaders/Effects.fx", 0, 0, "VS", "vs_4_0", 0, 0, &VSBuffer, 0);
...
struct pixel_shader
{
	...
	void Initialize(ID3D11Device* Device)
	{
		...
		HRESULT Result;
		ID3D10Blob* PSBuffer;
		Result = D3DCompileFromFile(L"Shaders/Effects.fx", 0, 0, "PS", "ps_4_0", 0, 0, &PSBuffer, 0);
...
struct character_vertex_shader
{
	ID3D11VertexShader* Shader;
	ID3D11InputLayout*	InputLayout;
	ID3D11Buffer*		ConstantBuffer;
	void Initialize(ID3D11Device* Device)
	{
		...
		uint32 InputCount = ARRAYSIZE(LayoutDescription);
		Result = D3DCompileFromFile(L"Shaders/Character.fx", 0, 0, "VS", "vs_4_0", 0, 0, &VSBuffer, 0);
		Result = Device->CreateVertexShader(VSBuffer->GetBufferPointer(), VSBuffer->GetBufferSize(), NULL, &Shader);
		Result = Device->CreateInputLayout(LayoutDescription, InputCount, VSBuffer->GetBufferPointer(), VSBuffer->GetBufferSize(), &InputLayout);
		VSBuffer->Release();
		...
		D3D11_BUFFER_DESC ConstantBufferDescription = { 0 };
		ConstantBufferDescription.Usage = D3D11_USAGE_DEFAULT;
		ConstantBufferDescription.ByteWidth = sizeof(DirectX::XMMATRIX) * 2;
		ConstantBufferDescription.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
		ConstantBufferDescription.CPUAccessFlags = 0;
		ConstantBufferDescription.MiscFlags = 0;
		Device->CreateBuffer(&ConstantBufferDescription, NULL, &ConstantBuffer);
...
struct character_pixel_shader
{
	...
	void Initialize(ID3D11Device* Device)
	{
		...
		HRESULT Result;
		ID3D10Blob* PSBuffer;
		Result = D3DCompileFromFile(L"Shaders/Character.fx", 0, 0, "PS", "ps_4_0", 0, 0, &PSBuffer, 0);
		Result = Device->CreatePixelShader(PSBuffer->GetBufferPointer(), PSBuffer->GetBufferSize(), NULL, &Shader);
		PSBuffer->Release();

		D3D11_BUFFER_DESC ConstantBufferDescription = { 0 };
		ConstantBufferDescription.Usage = D3D11_USAGE_DEFAULT;
		ConstantBufferDescription.ByteWidth = sizeof(directional_light);
		ConstantBufferDescription.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
		ConstantBufferDescription.CPUAccessFlags = 0;
		ConstantBufferDescription.MiscFlags = 0;
		Device->CreateBuffer(&ConstantBufferDescription, NULL, &ConstantBuffer);
